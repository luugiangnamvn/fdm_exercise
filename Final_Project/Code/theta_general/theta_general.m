...
for jj = 1:number_mesh
    ...
    r1 = a*dt/dx^2 - b*dt/(2*dx);
    r2 = -2*a*dt/dx^2;
    r3 = a*dt/dx^2 + b*dt/(2*dx);
    ...
    for it = 2:Nt    
        for i = 2:Nx
            if(i==2)
                F(i-1) = 0.5*f0(a,b,x(i),t(it))*dt+ 0.5*f0(a,b,x(i),t(it-1))*dt + r1*theta*g0(t(it)) + (1-theta)*r1*g0(t(it-1));
            elseif (i==Nx)
                F(i-1) = 0.5*f0(a,b,x(i),t(it))*dt+ 0.5*f0(a,b,x(i),t(it-1))*dt + r3*theta*g1(t(it)) + (1-theta)*r3*g1(t(it-1));
            else
                F(i-1) = 0.5*f0(a,b,x(i),t(it))*dt+ 0.5*f0(a,b,x(i),t(it-1))*dt;
