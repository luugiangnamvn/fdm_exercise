...
%% Create vector b    
    ...
    for i=1:N-1
        if(i==1)
            b(i)=functionf(i*del_x,cases) -(2*al)/(3*del_x) ;
        elseif(i==N-1)
            b(i)=functionf(i*del_x,cases) + (15*be)/(14*del_x) -(3*al)/(7*del_x) ;
        else
            b(i)=functionf(i*del_x,cases) -(2*al)/(3*del_x) ;
        end
    end
...
%% Create discrete solution with boundary 
    ...
    for i=1:N+1
        if (i==1)
            u_dis(i)=4/3*u(1,1)-1/3*u(2,1) -2/3*al*del_x;
        elseif(i==N+1)
            u_dis(i)=4/3*u(N-1,1)-1/3*u(N-2,1) +2/3*be*del_x;
        else
            u_dis(i)=u(i-1,1);
        end
    end
